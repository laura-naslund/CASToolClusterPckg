...
clusterGraphic <- function(clusters, pca1, flowlines, sites, STATE.map, map.title, file.name) {
  ...
  vars.max <- ggplot2::ggplot() +
    ...
    ggrepel::geom_text_repel(data = pca.vars.desc[1:10, ], size = 3,
                             ggplot2::aes(x = Dim.1, y = Dim.2, label = paste(vars))) +
    ggplot2::xlab("PC 1") +
    ggplot2::ylab("PC 2") +
    ggplot2::coord_equal() +
    ggplot2::theme_minimal() +
  ...
  if (map.title %in% c("Arizona", "Arkansas", "Colorado", "Connecticut",
    ...
    dev.off()
                              ...
                              "Vermont")) { # mostly vertical states (not tested with any)
